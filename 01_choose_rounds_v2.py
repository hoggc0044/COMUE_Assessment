...
class ChooseRounds:
    ...
    def __init__(self):
        ...
        # Flag to track if the custom rounds window is already open
        self.custom_window_open = False
        ...
        # Top error label (sits below input box when
        ...
        for item in range(0, 3):
            ...
            self.rounds_button.grid(row=0, column=item,
                                    padx=5, pady=5)
            self.buttons.append(self.rounds_button)
    ...
    def custom_rounds(self):
        ...
        if not self.custom_window_open:
            self.custom_window_open = True  # Set the flag to True when opening the window
            ...
            self.custom_window.protocol("WM_DELETE_WINDOW", self.on_custom_window_close)

    def on_custom_window_close(self):
        self.custom_window_open = False  # Reset the flag when the window is closed
    ...
    def submit_custom_rounds(self):
        ...
        try:
            ...
            if 1 <= num_rounds <= 100:
                print("You Chose {} rounds".format(num_rounds))
                self.custom_window_open = False  # Ensure the flag is reset when the window is destroyed
                self.custom_window.destroy()
                self.enable_buttons()
            else:
                has_error = "yes"
    ...
    def disable_buttons(self):
        for button in self.buttons[:2]:  # Only disable the "5" and "10" buttons
            button.config(state=DISABLED)

    def enable_buttons(self):
        for button in self.buttons[:2]:  # Only enable the "5" and "10" buttons
